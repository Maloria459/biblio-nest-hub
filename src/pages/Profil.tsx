...
// ─── Book grid card (reused for recommandations & coups de cœur) ───
function ProfileBookCard({ book, onClick }: { book: any; onClick?: () => void }) {
  return (
    <div
      className="group cursor-pointer rounded-xl border border-border bg-card overflow-hidden shadow-sm hover:shadow-md transition-shadow"
      onClick={onClick}
    >
...
const Profil = () => {
  ...
  // Visible tabs (own profile = all 5)
  const isOwnProfile = true; // future: compare with viewed user
